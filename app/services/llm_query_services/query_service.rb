module LlmQueryServices
  class QueryService < ::ApplicationService
    def initialize(*args)
      super(args)
      @system_templates = YAML.load_file(File.join(__dir__, 'llm_query_templates', 'system_prompts.yml'))
      @user_templates = YAML.load_file(File.join(__dir__, 'llm_query_templates', 'user_prompts.yml'))
    end
